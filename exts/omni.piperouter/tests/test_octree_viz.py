...
def test_leaves_partition_free_space_and_adapt():
    ...
    # every FREE cell lands in exactly one leaf; blocked cells in none
    assert (leaf_of[~occ] >= 0).all()
    assert (leaf_of[occ] == -1).all()
    # adaptive: big leaves exist in open air, 1-cell leaves hug the wall
...
def test_corridor_band_connects_and_is_a_small_fraction():
    ...
    for c in corr:                              # the band must contain the corridor
        assert band[c]
    assert not band.all()                       # ...and only a fraction of the grid
