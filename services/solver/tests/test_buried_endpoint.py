...
def test_buried_start_relocates_and_routes_with_note(empty_stack):
    ...
    s.occupancy[1, 1, 1] = 1                 # bury the start cell inside solid geometry
    frame = s.frame
    ...
    # A buried endpoint routes rather than failing, reports the relocation in the note,
    # and leaves the tube's first point outside the blocked cell.
...
def test_open_endpoints_route_with_no_note(empty_stack):
    ...
    assert res.status == "routed"
    assert res.note == ""                    # nothing buried, so nothing to warn about
