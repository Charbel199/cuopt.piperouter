...
def test_every_local_optimizer_pins_endpoints_and_stays_free(empty_stack):
    ...
    # A zig-zag through free cells: y alternates 5/6, and x=5 lands at y=6, so it clears
    # the blocked cell.
    ...
    for name, cls in optimizers.LOCAL_OPTIMIZERS.items():
        ...
        # An optimizer may densify the path, so check the invariants, not the point count.
